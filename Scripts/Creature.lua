...
function Creature:Creature(creatureIndex)
    ...
    self.icon = ScenePrimitive(ScenePrimitive.TYPE_PLANE, 0.04,0.04,0)
    ...
    self.slash = ScenePrimitive(ScenePrimitive.TYPE_PLANE, 0.15,0.15,0)
...
function Creature:Die()
    ...
    elseif self.boss == true then
        ...
        if random() < 0.1 then
             self.level:dropItem(self.x, self.z, "heart")
             return
        elseif random() < 0.3 then
             self.level:dropItem(self.x, self.z, "golds")
            return
        end       
...
function Creature:Think(e)
    ...
    if self.targetCreature ~= nil then
        local attackDistance = level.levelScale*2
        if self.thinkType == AI_ARCHER then
            attackDistance = level.levelScale*3
        end

        if self.boss == true then attackDistance = level.levelScale*5 end
...
--                if self.hurting == false then
  ...
  --              end
    ...
    if self.arrowCharge < 5 then 
        ...
        self.body.roll = 30 * (self.arrowCharge/5) * self.movDirMod
    end
    end
    ...
    if self.dead == true then
        if self.dying == true then
            self.body.xscale = 0.8
            self.body.yscale = 0.8
             self.body.roll = self.body.roll + 400 * e
             self.shield.roll = self.shield.roll + 100 * e
            self.shield.y = self.shield.y - e * 0.1

            self.body.y = self.body.y - e * 0.1
            self.body.x = self.body.x + e * 0.1
            if self.body.roll > 90 then
                self.dying = false
            end
        end
    else
    ...
    elseif self.moving == true then        
        ...
        self.body.y = (self.levelScale*0.225) + playerY
        self.body.yscale = 1 + (playerY * 20)
